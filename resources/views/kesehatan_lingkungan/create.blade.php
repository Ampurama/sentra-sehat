...
@section('title', 'Tambah Data Kesehatan Lingkungan Baru')
...
<div class="container mx-auto p-4 md:p-8">
    <div class="max-w-4xl mx-auto bg-white p-6 sm:p-8 rounded-3xl shadow-2xl">
        ...
        <header class="mb-8 border-b border-gray-200 pb-4">
            <h1 class="text-2xl sm:text-3xl font-extrabold text-blue-700">Formulir Tambah Data Kesehatan Lingkungan Baru</h1>
            <p class="text-sm text-gray-600 mt-2">Lengkapi informasi kesehatan lingkungan untuk ditambahkan ke sistem.</p>
        </header>
        ...
        <form action="{{ route('kesehatan_lingkungan.store') }}" method="POST">
            @csrf
            ...
            <section class="mb-8 p-4 sm:p-6 border border-blue-200 rounded-xl bg-blue-50/50 shadow-inner">
                    ...
                    <div>
                        ...
                        <select name="wilayah_id" id="wilayah_id" class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-3 border @error('wilayah_id') border-red-500 @enderror" required>
                            ...
                            @foreach($wilayahs as $wilayah)
                                <option value="{{ $wilayah->id }}" {{ old('wilayah_id') == $wilayah->id ? 'selected' : '' }}>{{ $wilayah->nama }}</option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                    <div>
                        ...
                        <input type="number" name="air_quality" id="air_quality" class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-3 border @error('air_quality') border-red-500 @enderror" value="{{ old('air_quality') }}" min="0" max="500">
                    ...
                    <div>
                        ...
                        <select name="sanitation_level" id="sanitation_level" class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-3 border @error('sanitation_level') border-red-500 @enderror">
                            ...
                            <option value="Baik" {{ old('sanitation_level') == 'Baik' ? 'selected' : '' }}>Baik</option>
                            <option value="Sedang" {{ old('sanitation_level') == 'Sedang' ? 'selected' : '' }}>Sedang</option>
                            <option value="Buruk" {{ old('sanitation_level') == 'Buruk' ? 'selected' : '' }}>Buruk</option>
                        </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                    <div>
                        ...
                        <select name="waste_management" id="waste_management" class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-3 border @error('waste_management') border-red-500 @enderror">
                            ...
                            <option value="Baik" {{ old('waste_management') == 'Baik' ? 'selected' : '' }}>Baik</option>
                            <option value="Sedang" {{ old('waste_management') == 'Sedang' ? 'selected' : '' }}>Sedang</option>
                            <option value="Buruk" {{ old('waste_management') == 'Buruk' ? 'selected' : '' }}>Buruk</option>
                        </select>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-3 border @error('description') border-red-500 @enderror" rows="4" placeholder="Deskripsi kondisi lingkungan">{{ old('description') }}</textarea>
            ...
            <div class="mt-10 flex flex-col-reverse sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-4">
                <a href="{{ route('kesehatan_lingkungan.index') }}" class="w-full sm:w-auto px-6 py-3 border border-gray-300 rounded-xl text-gray-700 hover:bg-gray-100 transition duration-300 text-center">Batal</a>
